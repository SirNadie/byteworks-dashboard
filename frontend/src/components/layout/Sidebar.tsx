...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
    ...
    const getLinkClass = (path: string) => {
        const base = "nav-item flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 group";
    ...
    return (
        <>
            ...
            {/* Sidebar Container */}
            <aside className={`
                w-64 bg-white dark:bg-card-dark border-r border-gray-200 dark:border-border-dark 
                flex flex-col fixed inset-y-0 left-0 z-50 transition-transform duration-300 ease-in-out
            ...
            `}>
                ...
                <nav className="flex-1 px-4 space-y-2 mt-4">
                    ...
                    <Link href="/contacts" className={getLinkClass('/contacts')} onClick={() => onClose()}>
                        <span className="material-symbols-outlined">group</span>
                        <span className="text-sm font-medium">Contacts</span>
                    </Link>
                    <Link href="/quotes" className={getLinkClass('/quotes')} onClick={() => onClose()}>
                        <span className="material-symbols-outlined">description</span>
                        <span className="text-sm font-medium">Quotes</span>
                    </Link>
                    <Link href="/invoices" className={getLinkClass('/invoices')} onClick={() => onClose()}>
                ...
                <div className="p-4 border-t border-gray-200 dark:border-border-dark relative">
                    <button
                        ...
                        className="w-full flex items-center gap-3 p-2 rounded-xl hover:bg-gray-50 dark:hover:bg-border-dark cursor-pointer transition-colors group text-left"
                    >
                        <div className="size-10 rounded-full bg-brand flex items-center justify-center text-white font-bold shadow-md uppercase">
                    ...
                    {/* Dropdown Menu */}
                    {
                        isUserMenuOpen && (
                            <>
                                ...
                                <div className="absolute bottom-full left-4 right-4 mb-2 bg-white dark:bg-card-dark rounded-xl shadow-lg border border-gray-200 dark:border-border-dark overflow-hidden z-50 animate-in slide-in-from-bottom-2 fade-in duration-200">
